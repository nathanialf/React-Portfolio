...
(async () => {
  ...
  try {
    // 1) INDEX  +  switch to dark mode first (footer toggle at bottom)
    log('STEP 1: index / homepage, then enable dark mode');
    await page.goto(BASE + '/', { waitUntil: 'networkidle' });
    await page.mouse.move(cx, cy);
    await pause(1800);
    await shot(page, 'index-home-light');

    // scroll to the footer and flip the theme to Dark before anything else
    const pageBottom = await page.evaluate(() => document.body.scrollHeight);
    await smoothScroll(pageBottom, 1900);
    await shot(page, 'footer-theme-toggle');
    const darkBtn = page.getByRole('button', { name: /dark mode/i }).first();
    await click(darkBtn, 650);
    await pause(1300);
    await shot(page, 'index-dark');
    await smoothScroll(0, 1700);
    await shot(page, 'index-top-dark');
